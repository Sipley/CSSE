...
def convertAngleFromDeg(angle=None):
    degree = int(angle.split('d')[0])
    arcminute = float(angle.split('d')[1])/60
    convertedAngle = degree + arcminute
    print convertedAngle
#convertAngleFromDeg('100d42.6')

def convertAngleToDeg(angle=None):
    degree = angle.split('.')[0]
    arcminute = round(((int(angle.split('.')[1]))*.01)*60,2)
    print arcminute
    convertedAngle = str(degree)+'d'+arcminute
    print convertedAngle

convertAngleToDeg('100.71')
...
#    print diffYear
#    cumProgression = diffYear * convertAngle()
#    print math
